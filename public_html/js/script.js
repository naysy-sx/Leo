...
var swiper = new Swiper("#buy-locations", {
    slidesPerView: 4,
    loop: true,
    clickable: true,
    spaceBetween: 30,
...
document.querySelectorAll("[data-countdown]").forEach(function (elem) {
    ...
    const countDownInterval = setInterval(function () {
        ...
        elem.innerHTML =
            countDownItem(daysValue, "дней") +
            countDownItem(hoursValue, "часов") +
            countDownItem(minutesValue, "минут") +
            countDownItem(secondsValue, "секунд");
...
//newsletterPopup(); todo: всплывашка

function filtrationData() {
    var seasons_name = $('.seasons-select').val();
    var genders_name = $('.genders-select').val();
    var sizes_name = $('.sizes-select').val();
    var colors_name = $('.colors-select').val();

    console.log(seasons_name + "|" + genders_name + "|" + sizes_name + "|" + colors_name)

    $.get("index.php?r=shop/products-search", {seasons_name, genders_name, sizes_name, colors_name}, function (res) {
        console.log('---');
        console.log(res);
        console.log('---');
...
$('#catalog-filters').on("change", (e) => {
    console.log('!!')
    filtrationData();
})


$('.add-to-card').on("click", (e) => {
    var cr = e.target;
    var id = $(cr).data('id');

    $.get("index.php?r=cart/add-to-cart", {id}, function (res) {
        var prod_qty = parseInt($('.items__count').text());
        $('.items__count').text(++prod_qty);

        $('.cart-refresh').html(res);
...
$('.delete-card').on("click", function(e){
    e.preventDefault();
    $(this).parent('.minicart__product--items').remove();
/*
    $.get("index.php?r=cart/add-to-cart", {id}, function (res) {
        var prod_qty = parseInt($('.items__count').text());
        $('.items__count').text(prod_qty--);
...
$('.order-form').on('change', function(){
    ...
    if($('#order-delivery').is(':checked')){
        $('#i-1').css('display','flex');
        $('.order-item.-self-delivery').hide();
    }
    if($('#order-delivery-points').is(':checked')){
        $('#i-2').css('display','flex');
        $('.order-item.-self-delivery').hide();
    }
    if($('#self-delivery').is(':checked')){
        $('#i-3').css('display','flex');
        $('.order-item.-self-delivery').show();
    }
    if($('#boxberry, #sdek').is(':checked')){
        $('.order-address').css('display', 'flex');
    }else{
        $('.order-address').hide();
    }
})
